    ...
    public partial class HistorialEvaluaciones : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (Session["rol"] != null && Session["rol"].ToString().ToLower() != "admin")
                {
                    lnkEliminarEvaluacion.Visible = false;
                }
                //this.DataBind();
            }
        }
        ...
        private void CargarAnios()
        {
            ...
            {
                conn.Open();
                //Solo considerar periodos con evaluaciones completadas
        ...
        {
            //Se cargan los empleados que tienen evaluaciones completadas y evaluadas
            DataTable dt = new DataTable();
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                conn.Open();
                string query = @"SELECT DISTINCT emp.id_empleado, (emp.nombre + ' ' + emp.apellido) AS nombreCompleto
                                 FROM Evaluaciones e
        ...
        private void CargarTrimestresDisponibles(int anio, int idEmpleado)
        {
            ...
            {
                conn.Open();
                //Consulta: obtener trimestres donde exista una evaluación completada (finalizada=1 y EvaluadaSupervisor=1)
                string query = @"
        ...
        protected void ddlAnios_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            //Si ya hay un trimestre seleccionado, intenta cargar el historial:
            CargarHistorial();
        }
        ...
        private void CargarHistorial()
        {
            ...
            //Consultar el detalle de la evaluación para el empleado, año y trimestre seleccionados
            DataTable dt = new DataTable();
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                conn.Open();
                //Se unen Evaluaciones, EvaluacionesPreguntas, Preguntas y PeriodosEvaluacion
                string query = @"
        ...
        protected void lnkEliminarEvaluacion_Click(object sender, EventArgs e)
        {
            ...
            if (!int.TryParse(ddlAnios.SelectedValue, out anio) || anio == 0)
            {
                litMensaje.Text = "<div class='alert alert-warning'>Seleccione un año válido.</div>";
                return;
            }
            if (!int.TryParse(ddlEmpleados.SelectedValue, out idEmpleado) || idEmpleado == 0)
            {
                litMensaje.Text = "<div class='alert alert-warning'>Seleccione un empleado válido.</div>";
                return;
            }
            if (!int.TryParse(ddlTrimestres.SelectedValue, out trimestre) || trimestre == 0)
            {
                litMensaje.Text = "<div class='alert alert-warning'>Seleccione un trimestre válido.</div>";
                return;
            }

            //Obtener el id de la evaluación a partir de los filtros
            int idEvaluacion = ObtenerIdEvaluacionPorFiltros(anio, idEmpleado, trimestre);
            if (idEvaluacion == 0)
            {
                litMensaje.Text = "<div class='alert alert-warning'>No se encontró la evaluación para esos filtros.</div>";
                return;
            }
            ...
            if (Session["rol"] == null || Session["rol"].ToString().ToLower() != "admin")
            {
                litMensaje.Text = "<div class='alert alert-danger'>No tiene permisos para eliminar evaluaciones.</div>";
                return;
            }
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                using (SqlTransaction tran = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        string deletePreguntas = "DELETE FROM EvaluacionesPreguntas WHERE id_evaluacion = @idEvaluacion";
                        using (SqlCommand cmd = new SqlCommand(deletePreguntas, conn, tran))
                        {
                            cmd.Parameters.AddWithValue("@idEvaluacion", idEvaluacion);
                            cmd.ExecuteNonQuery();
                        }

                        //Luego, eliminar la evaluación en sí
                        string deleteEvaluacion = "DELETE FROM Evaluaciones WHERE id_evaluacion = @idEvaluacion";
                        using (SqlCommand cmd = new SqlCommand(deleteEvaluacion, conn, tran))
                        {
                            cmd.Parameters.AddWithValue("@idEvaluacion", idEvaluacion);
                            cmd.ExecuteNonQuery();
                        }
                    ...
                    {
                        tran.Rollback();
                        litMensaje.Text = "<div class='alert alert-danger'>Error al eliminar la evaluación: " + ex.Message + "</div>";
                    }
                }
            }
        }

        private int ObtenerIdEvaluacionPorFiltros(int anio, int idEmpleado, int trimestre)
        {
            int idEvaluacion = 0;
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
